var pSDK = function ({ actions, api, app }) {
  var self = this;
  ...
  var storages = _.map(dbmeta, (v, i) => {
    return i;
  });

  var dbversion = 2 + storages.length;

  self.db = new ResoursesDB(
    "psdk_" + (window.testpocketnet ? "test" : "production"),
    dbversion,
    storages
  );
  ...
  var loadList = function (
    ...
    _.each(keys, k => {
      if (p.alternativeGetStorage) {
        ...
        if (
          !p.update &&
          storage[p.alternativeGetStorage][k] &&
          !storage[p.alternativeGetStorage][k].___temp
        ) {
    ...
    var promise = !load.length
      ? Promise.resolve([])
      : new Promise((resolve, reject) => {
          (p.update ? Promise.resolve([]) : getfromdb(p.indexedDb, load)).then(
            dbr => {
              ...
              var rjc = function (e) {
                getfromdb(p.indexedDb, load, true)
                  .then(r => {
  ...
  var request = function (
    ...
    temp[key][hash] = getfromdbone(p.requestIndexedDb, hash)
      .then(r => {
      ...
      _.each(temps, k => {
        _.each(account.getTempActions(k), alias => {
          if (k == type) {
            if (helpId) {
              if (
                !object &&
                (alias.id == helpId ||
                  alias.actionId == helpId ||
                  (k == "userInfo" && alias.actor == helpId))
              ) {
          ...
          if (self[k] && self[k].applyAction) {
            ...
            var applied = self[k].applyAction(
              extendedObject || object.clone(),
              alias
            );
  ...
  self.userInfo = {
      ...
      return _.filter(
        _.map(rawinfo, c => {
          try {
            ...
            c.i = checkIfAllowedImageApply(
              clearStringXss(trydecode(c.i || ""))
            );
    ...
    findlocal: function (finder) {
      return _.find(
        _.toArray(objects["userInfoFull"]).concat(objects["userInfoLight"]),
        finder
      );
    },
    ...
    get: function (address) {
      return this.tempExtend(
        objects["userInfoFull"][address] || objects["userInfoLight"][address],
        address
      );
    },
    ...
    getclear: function (address) {
      return (
        storage["userInfoFull"][address] || storage["userInfoLight"][address]
      );
    },
    ...
    getmyoriginal: function () {
      ...
      return (
        objects["userInfoFull"][app.user.address.value] ||
        objects["userInfoLight"][app.user.address.value]
      );
    },
    ...
    getShortForm: function (address) {
      ...
      var name =
        app.platform.api.clearname(userInfo.name || address) || address;
    ...
    insertFromResponse: function (data, light) {
      ...
      return settodb(indexedDb, result)
        .then(() => {
    ...
    insertFromResponseSmall: function (data, light) {
      ...
      _.each(result, r => {
        if (!storage[key][r.key]) storage[key][r.key] = r.data; // ADD STORAGE

        var object = this.transform(r);

        if (object && !objects[key][r.key]) objects[key][r.key] = object;
      });
    ...
    listener: function (exp, address, status) {
      if (status == "completed") {
        objects["userInfoFull"][address] = this.applyAction(
          objects["userInfoFull"][address],
          exp
        );
    ...
    load: function (addresses, light, update) {
      return loadList(
        light ? "userInfoLight" : "userInfoFull",
        addresses,
        addresses => {
  ...
  self.userState = {
    ...
    load: function (addresses, update) {
      return loadList(
        ...
        addresses => {
          return api
            .rpc("getuserstate", [addresses.join(",")])
            .then(d => {
  ...
  self.accSet = {
    ...
    tempExtend: function (object, address) {
      return extendFromActions("accSet", ["accSet"], object, address);
    },
  ...
  self.comment = {
          ...
          if (object.txid == exp.postid) {
            if (exp.optype == "comment") {
              ...
              var last = object.lastComment
                ? self.comment.get(object.lastComment)
                : null;
            ...
            if (exp.optype == "commentDelete") {
              if (object.comments > 0) object.comments--;
        ...
        _.map(rawcomments, c => {
          ...
          try {
            ...
            if (_.isObject(c.msgparsed)) {
              c.msgparsed.url = clearStringXss(
                trydecode(c.msgparsed.url || "")
              );

              c.msgparsed.message = clearStringXss(
                trydecode(c.msgparsed.message || "")
              ).replace(/\n{2,}/g, "\n\n");

              c.msgparsed.images = _.filter(
                _.map(c.msgparsed.images || [], function (i) {
    ...
    listener: function (exp, address, status) {
      if (status == "completed") {
        if (exp.optype == "comment") {
          objects["comment"][exp.id] = this.applyAction(
            objects["comment"][exp.id] || exp,
            exp
          );
        } else {
    ...
    load: function (ids, update) {
      return loadList(
        ...
        ids => {
          return api
            .rpc("getcomments", ["", "", app.user.address.value || "", ids])
            .then(d => {
              d = this.cleanData(d);

              checkObjectInActions(
                _.map(d, c => {
                  return {
                    txid: c.id
                  };
    ...
    tempExtend: function (object, id) {
      return extendFromActions("comment", ["comment", "cScore"], object, id);
    },
  ...
  self.cScore = {
    ...
    listener: function (exp, address, status) {
      if (status == "completed") {
        objects["comment"][exp.comment.v] = this.applyAction(
          objects["comment"][exp.comment.v],
          exp
        );
  ...
  self.share = {
      ...
      return _.filter(
        _.map(rawshares, c => {
          ...
          try {
            ...
            c.i = _.filter(
              _.map(c.i || [], function (i) {
                return checkIfAllowedImageApply(clearStringXss(i));
              }),
              function (i) {
                return i;
              }
    ...
    insertFromResponseEx: function (response) {
      self.userInfo.insertFromResponse(
        self.userInfo.cleanData(response.users),
        true
      );
    ...
    insertFromResponseSmall: function (data, ncn) {
      if (!ncn) data = this.cleanData(data);
      ...
      _.each(result, r => {
        if (
          !storage[key][r.key] ||
          (storage[key][r.key] && storage[key][r.key].___temp)
        )
          storage[key][r.key] = r.data; // ADD STORAGE
    ...
    load: function (txids, update) {
      return loadList(
        ...
        txids => {
          return api
            .rpc("getrawtransactionwithmessagebyid", [txids])
            .then(d => {
    ...
    transform: function ({ data: share, key }, small) {
      if (share.userprofile || share.user) {
        self.userInfo[
          !small ? "insertFromResponse" : "insertFromResponseSmall"
        ](self.userInfo.cleanData([share.userprofile || share.user]), true);
      }

      if (share.lastComment) {
        self.comment[!small ? "insertFromResponse" : "insertFromResponseSmall"](
          self.comment.cleanData([share.lastComment])
        );
      }
  ...
  self.subscribe = {
    ...
    tempAdd: function (objects = [], filter) {
      _.each(actions.getAccounts(), account => {
        var actions = _.filter(
          account.getTempActions("subscribePrivate"),
          filter
        ).concat(_.filter(account.getTempActions("subscribe"), filter));
  ...
  self.contentDelete = {
    ...
    listener: function (exp, address, status) {
      if (status == "completed") {
        objects["share"][exp.txidEdit] = this.applyAction(
          objects["share"][exp.txidEdit],
          exp
        );
  ...
  self.upvoteShare = {
    ...
    listener: function (exp, address, status) {
      if (status == "completed") {
        objects["share"][exp.share.v] = this.applyAction(
          objects["share"][exp.share.v],
          exp
        );
    ...
    load: function (shareIds, commentIds, update) {
      ...
      return loadList(
        ...
        ids => {
          ...
          return api
            .rpc("getpagescores", [sIds, app.user.address.value, cIds])
            .then(data => {
              return _.map(ids, id => {
                return {
                  data:
  ...
  self.transaction = {
    ...
    load: function (id, update, p) {
      return loadone(
        ...
        ids => {
          return api
            .rpc("getrawtransaction", [ids[0], 1], { rpc: p })
            .then(d => {
              if (_.isEmpty(d)) {
                return [];
              }
              ...
              if (!d.confirmations) {
                ...
                if (d.height) {
                  app.platform.currentBlock
                    ? (d.confirmations = Math.max(
                        app.platform.currentBlock - d.height,
                        0
                      ))
                    : null;
                } else {
                  app.platform.currentBlock
                    ? (d.height = app.platform.currentBlock)
                    : null;
                }
              }
  ...
  self.blocking = {
    ...
    load: function (address, update) {
      return loadone(
        ...
        ids => {
          return api
            .rpc("getuserblockings", [ids[0], "1", "", "", 0, 5000], {
              rpc: {
  ...
  self.subscribers = {
    ...
    load: function (address, update) {
      return loadone(
        ...
        ids => {
          return api
            .rpc("getusersubscribers", [ids[0], "", "", 0, 5000])
            .then(r => {
              r = _.map(r, v => {
                return v.address;
              });
  ...
  self.subscribes = {
    ...
    load: function (address, update) {
      return loadone(
        ...
        ids => {
          return api
            .rpc("getusersubscribes", [ids[0], "", "", 0, 5000])
            .then(r => {
              r = _.map(r, v => {
                return {
                  adddress: v.adddress,
        ...
        _.map(data, tg => {
          ...
          try {
            t = {
              count: tg.count,
              tag: clearTagString(
                trim(decodeURIComponent(decodeURIComponent(tg.tag)))
              )
            };
          } catch (e) {
            //console.log(tg, e)
  ...
  self.actions.on("actionFiltered", ({ action, address, status }) => {
    ...
    if (address == app.user.address.value) {
      ...
      if (status == "completed" && action.object.ustate) {
        var ustate =
          typeof alias.ustate == "function" ? alias.ustate() : alias.ustate;
    ...
    update: function (type, wsdata) {
      ...
      if (alias) {
        _.each(self.updatelisteners, l => {
          l({ alias, status, type }, "updateListener");
        });
      }
